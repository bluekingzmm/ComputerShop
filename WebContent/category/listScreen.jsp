					...
					<div id="myTabContent" class="tab-content">
						<div class="tab-pane active in" id="home"><c:forEach
								...
								<form id="tab" method="post"
									action='<c:url value="/ScreenAction?operate=modify"></c:url>'>
									<input type="hidden" name="id" value="${s.screen_id }" /> <label>屏幕尺寸</label>
									<input type="text" value="${s.screen_name}" class="input-xlarge"
									name="name"> <input type="submit" value="更新" />
								</form>
								<a
									href="<c:url value="/ScreenAction?operate=delete&id=${s.screen_id}"></c:url>">删除</a>
							<br>
							</c:forEach>
							<form action="<c:url value="/ScreenAction"></c:url>" method="post"><input
								type="hidden" name="operate" value="add"> <input
